---
{
  "title": "A decision record is not a changelog",
...
Those sound like the same thing. They are not, and the gap between them is why the same argument reopens every quarter with the same people reaching the same conclusion, having lost the reasoning that got them there the first time.

## The thing that gets lost

A backlog is a snapshot of a conclusion. It tells you that self-serve onboarding is ranked first and SSO is ranked third. What it does not tell you — what it structurally cannot tell you — is:
...
**The choice.** What you are doing, stated so a stranger could tell whether it happened.
...
**The tripwire.** The condition that should make you revisit this. Sometimes it is a metric, sometimes a date, sometimes an event — *if two more enterprise deals cite SSO, re-score it.* A decision without a tripwire is a decision you will either never revisit or revisit at random.
...
Writing down what you rejected is the highest-leverage part of the whole exercise, because it is the only part that decays completely if you don't.

## What this is not

**It is not a changelog.** A changelog records what shipped, after it shipped. A decision record is written at the moment of choosing, before the outcome is known, and its value comes precisely from being uncontaminated by hindsight. If you write it afterwards you will write down the reasoning that turned out to be right, which is not the reasoning you had.

**It is not a postmortem.** Postmortems examine outcomes. Decision records examine the choice under the information available at the time. A decision can be good and the outcome bad; you cannot tell those apart later without a record of what you knew.

**It is not a document nobody reads.** If it takes more than five minutes to write or more than thirty seconds to read, it will not survive contact with a real sprint. Length is the enemy here.
...
None of that is visible from a backlog, because a backlog only ever shows the current state. It is visible from a sequence of decisions with their reasoning attached — which is the same reason the last step of a prioritization loop should feed the first.

That is the whole argument for treating prioritization as a system rather than a recurring spreadsheet: not that scoring is hard, but that a decision you can explain is a decision you can learn from, and a decision you can only re-derive is one you will pay for repeatedly.
