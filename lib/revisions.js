...
export async function resolveActor(userId) {
  try {
    // clerkClient is `() => Promise<ClerkClient>` in @clerk/nextjs v6
    // (verified against the installed 6.39.1 types). Earlier drafts also
    // handled it being exported as a plain object; that shape does not exist
    // in this major, so the branch was dead code and is gone.
