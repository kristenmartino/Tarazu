    ...
    <div style={{ padding: 20, border: `1px solid ${C.borderActive}`, borderRadius: 12, background: C.surface }}>
      <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          ...
          <span style={{ fontSize: 10, color: C.textMuted, fontFamily: "var(--mono)", padding: "2px 8px", background: C.border, borderRadius: 4 }}>{format}</span>
        </div>
        {!hasRice && (
          <div style={{ padding: "8px 12px", borderRadius: 8, background: C.warnDim, border: `1px solid ${C.warn}30` }}>
            <span style={{ fontSize: 10, color: C.warn, fontFamily: "var(--mono)" }}>RICE scores not found in CSV — all values will default to 50. You can edit them after import.</span>
          </div>
        )}
        <div style={{ maxHeight: 240, overflowY: "auto", display: "flex", flexDirection: "column", gap: 6 }}>
          {features.map((f, i) => (
            <div key={i} style={{ padding: "8px 12px", borderRadius: 8, border: `1px solid ${C.border}`, background: C.bg }}>
              ...
              {hasRice && <div style={{ fontSize: 9, color: C.textDim, marginTop: 4, fontFamily: "var(--mono)" }}>R:{f.reach} I:{f.impact} C:{f.confidence} E:{f.effort}</div>}
            </div>
          ))}
        </div>
        <div style={{ display: "flex", gap: 10 }}>
          <button onClick={onConfirm} style={{ flex: 1, padding: "10px 16px", border: "none", borderRadius: 8, background: C.accent, color: C.bg, fontSize: 13, fontWeight: 700, cursor: "pointer", fontFamily: "var(--mono)" }}>Import {features.length} Candidates</button>
